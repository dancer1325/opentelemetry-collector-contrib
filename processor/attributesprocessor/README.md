...
* attributes processor 
  * allows
    * modifying 
      ...
      * log's attributes
      * metric's attributes
    * filtering and matching input data
      * == [include OR exclude](#includeexclude-filtering) / specified actions
  * [config spec](./config.md)
    * actions
      * compose MULTIPLE / ⚠️order specified is important⚠️
      ...
      * SUPPORTED ones
        - `insert`
          - if input data's key does NOT exist -> insert a NEW attribute 
          - ⚠️requirements⚠️
            - `key`
            - `value` OR `from_attribute` OR `from_context`
        ...
        - `update`
          - if input data's key ALREADY exists -> update an attribute
          - ⚠️requirements⚠️
        ...
        - `upsert`
          ...
          - ⚠️requirements⚠️
            - == `insert`'s requirements
        - `delete`
          - deletes an input data's attribute 
          - requirements
            - ⚠️`key` &/ OR `pattern`⚠️
        ...
        - `hash`
          - existing attribute value's hashes (SHA1)
          - requirements
            - ⚠️`key` &/ OR `pattern`⚠️
        ...
        - `extract`
          - TODO:Extracts values using a regular expression rule from the input key
            to target keys specified in the rule
          - If a target key already exists, it will
            be overridden
          - Note: It behaves similar to the Span Processor `to_attributes`
    ...
    * _Examples:_ [here](./testdata/config.yaml)
